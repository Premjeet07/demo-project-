...
//	}
	...
	public String fetchClaimDetails() {
		...
		if (claim != null && claim.getSubscribe() != null) {
			...
			// If hId via recipient, use getCoverageAmountByHid; if subscribeId is more
			// accurate, use that.
			if (subscribeId != null) {
				remainingCoverageAmount = processClaimDAO
						.getCoverageAmountBySubscribeId(claim.getSubscribe().getSubscribeId());
			}
			// OR if you want to fetch by subscribeId:
			// remainingCoverageAmount =
			// processClaimDAO.getRemainingCoverageAmountBySubscribeId(claim.getSubscribe().getSubscribeId());
		} else {
	...
	public String processClaim() {
		...
		if (status == null || status.trim().isEmpty()) {
			context.addMessage("form:status",
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Please choose one radio button", null));
			isValid = false;
		}
		if ("APPROVED".equals(status) && claim.getAmountClaimed() > getRemainingCoverageAmount()) {
			context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
					" The Amount Claimed Exceeds The Available Coverage:", null));
			isValid = false;
		}
		if ("APPROVED".equals(status) && (approveReason == null || approveReason.trim().isEmpty())) {
			context.addMessage("form:approveReason",
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Please choose one reason", null));
			isValid = false;
		}
		if ("DENIED".equals(status) && (denyReason == null || denyReason.trim().isEmpty())) {
			context.addMessage("form:denyReason",
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Please choose one denied reason", null));
			isValid = false;
		}
		if (("APPROVED".equals(status) && "Other".equals(approveReason)
				&& (otherReason == null || otherReason.trim().isEmpty()))
				|| ("DENIED".equals(status) && "Other".equals(denyReason)
						&& (otherReason == null || otherReason.trim().isEmpty()))) {
			context.addMessage("form:otherReasonTextArea",
	...
	public void onStatusChange(ValueChangeEvent event) {
		this.status = (String) event.getNewValue();

		// Reset dropdown model values
		this.approveReason = "";
		this.denyReason = "";
		this.otherReason = "";

		// Also reset UI component local values so old selections don't reappear
		FacesContext fc = FacesContext.getCurrentInstance();
		resetUIInput("form:approveReason", fc);
		resetUIInput("form:denyReason", fc);
		resetUIInput("form:otherReasonTextArea", fc);

		fc.renderResponse();
	}

	private void resetUIInput(String clientId, FacesContext fc) {
		UIComponent comp = fc.getViewRoot().findComponent(clientId);
