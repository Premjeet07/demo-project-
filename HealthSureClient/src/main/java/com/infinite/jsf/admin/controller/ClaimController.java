      ...
      boolean isValid=true;
		try {
			...
			if (searchType == null || searchType.isEmpty()) {
				context.addMessage("red:searchType",
			...
			case "subscribeId":
				if (searchValue == null || searchValue.isEmpty()) {
					context.addMessage("red:subscribeIdField",
							new FacesMessage(FacesMessage.SEVERITY_ERROR, "Subscribe ID is required.", null));
					return null;
				}
				if (!searchValue.matches("^SUB\\d+$")) {
					context.addMessage("red:subscribeIdField", new FacesMessage(FacesMessage.SEVERITY_ERROR,
			...
			case "providerId":
				if (searchValue == null || searchValue.isEmpty()) {
					context.addMessage("red:providerIdField",
							new FacesMessage(FacesMessage.SEVERITY_ERROR, "Provider ID is required.", null));
					return null;
				}
				if (!searchValue.matches("^P\\d+$")) {
					context.addMessage("red:providerIdField", new FacesMessage(FacesMessage.SEVERITY_ERROR,
			...
			case "hId":
				if (searchValue == null || searchValue.isEmpty()) {
					context.addMessage("red:recipientIdField",
							new FacesMessage(FacesMessage.SEVERITY_ERROR, "Recipient HID is required.", null));
					return null;
				}
				if (!searchValue.matches("^H\\d+$")) {
					context.addMessage("red:recipientIdField", new FacesMessage(FacesMessage.SEVERITY_ERROR,
			...
			case "status":
				if (searchValue == null || searchValue.isEmpty()) {
					context.addMessage("red:statusField",
							new FacesMessage(FacesMessage.SEVERITY_ERROR, "Claim Status is required.", null));
					return null;
				}
			...
			case "dateRange":
				...
				try {
					...
					if (userFromDate == null || userFromDate.isEmpty()) {
						context.addMessage("red:fromDate",
								new FacesMessage(FacesMessage.SEVERITY_ERROR, "from date is required.", null));
						isValid=false; 
					}
					if (userToDate == null || userToDate.isEmpty()) {
						context.addMessage("red:toDate",
								new FacesMessage(FacesMessage.SEVERITY_ERROR, "Todate is required.", null));
						isValid=false; 
...
if(!isValid) {
	return null;
					...
					context.addMessage("red:fromDate", new FacesMessage(FacesMessage.SEVERITY_ERROR,
			...
			default:
				context.addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_ERROR, "Unknown search type selected.", null));
				return null;
			}
			...
			if (claimsList == null || claimsList.isEmpty()) {
				context.addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_WARN, "No claims found for the given criteria.", null));
				return null;
			}
...
//        selectedClaimsList = null;
	...
	private void sortClaimsList() {
		...
		claimsList.sort((c1, c2) -> {
			...
			switch (sortColumn) {
			case "claimId":
				cmp = c1.getClaimId().compareTo(c2.getClaimId());
				break;
			case "providerId":
				cmp = c1.getProvider().getProviderId().compareTo(c2.getProvider().getProviderId());
				break;
			case "subscribeId":
				cmp = c1.getSubscribe().getSubscribeId().compareTo(c2.getSubscribe().getSubscribeId());
				break;
			case "status":
				cmp = c1.getClaimStatus().name().compareTo(c2.getClaimStatus().name());
				break;
			case "claimDate":
				cmp = c1.getClaimDate().compareTo(c2.getClaimDate());
				break;
			case "amountClaimed":
				cmp = Double.compare(c1.getAmountClaimed(), c2.getAmountClaimed());
				break;
			case "amountApproved":
				cmp = Double.compare(c1.getAmountApproved(), c2.getAmountApproved());
				break;
			case "hId":
				cmp = c1.getRecipient().gethId().compareTo(c2.getRecipient().gethId());
				break;
			}
